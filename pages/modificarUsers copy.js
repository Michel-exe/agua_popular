...
window.addEventListener("submit",async e => {
    ...
    let t = e.target
    let [id, type , inp] = [
        t.parentElement.getAttribute("data-id"),
        t.getAttribute("data-type"),
        t[0].value
    ];
    let da = new FormData();
    da.append("desEne",t[0].value);
    da.append("desFeb",t[1].value);
    da.append("desMar",t[2].value);

    if(type=="inputRange"){
        await fetch("../php/updateDescuentos.php",{
            method:"POST",
            body:da
        })  .then(res => res.text())
            .then(r=>{
